...
export default function PvpRecap({
    ...
    const a = useMemo(() => analyze(playerTeam, enemyTeam, won), [playerTeam, enemyTeam, won])
            ...
            <div style={S.card} onClick={(e) => e.stopPropagation()}>
                ...
                <div style={{ ...S.awards, ...sec(2) }}>
                    ...
                    <div style={{ ...S.award, ...S.flop }}>
                        <div style={S.awardTag}>{a.foeTag}</div>
                        <img src={dexSrc(a.foe.speciesId)} alt="" style={{ ...S.sprite, ...(won ? S.haloRed : S.haloBourreau) }} />
                        <div style={S.awardName}>{nameOf(a.foe)}</div>
                        <div style={S.awardSub}>{a.foeLine}</div>
                    </div>
                </div>
                ...
                <div style={{ ...S.block, ...S.analysis, ...sec(3) }}>
                    ...
                    <div style={S.bullet}><b>{a.whyLabel} :</b> {a.why}</div>
                </div>
...
interface Recap {
    intro: string; top3: string[]
    mvp: BattleMon; mvpLine: string
    foe: BattleMon; foeTag: string; foeLine: string   // 2e award : 🤡 PAILLASSON (win) ou 💀 BOURREAU (loss)
    what: string; why: string; whyLabel: string
}

// Effet de type d'un Daemon (type primaire) contre l'équipe en face (son lead) → leçon tactique.
function lesson(attacker: BattleMon, defenderLead: BattleMon | undefined, won: boolean): string {
    const atkType = (getSpecies(attacker.speciesId)?.types[0] ?? "NORMAL") as PokeType
    const dTypes = (getSpecies(defenderLead?.speciesId ?? "")?.types ?? []) as PokeType[]
    const mult = dTypes.length ? typeEffectiveness(atkType, dTypes) : 1
    const leadName = nameOf(defenderLead ?? attacker)
    if (won) {
        return mult >= 2
            ? `Type ${atkType} ×${mult} contre ${leadName} (${dTypes.join("/")}) → COUNTER élémentaire parfait. 🎓 La leçon : repère la faiblesse de type adverse et frappe pile là — un ×2 vaut deux tours d'avance.`
            : mult < 1
                ? `Même à contre-type (×${mult}), le BURST brut a fait la diff. 🎓 La leçon : une grosse stat offensive force le passage — mais c'est risqué, garde un counter en réserve.`
                : `Le coup qui a fait pencher la balance au bon moment (snowball). 🎓 La leçon : quand l'adversaire recule, enchaîne ton burst avant qu'il ne se soigne ou ne switch.`
    }
    return mult >= 2
        ? `Type ${atkType} ×${mult} contre ton ${leadName} (${dTypes.join("/")}) → il a EXPLOITÉ ta faiblesse de type. 🎓 La leçon : anticipe le matchup et switch sur un Daemon qui encaisse ce type.`
        : mult < 1
            ? `Même à contre-type (×${mult}), sa PUISSANCE brute t'a dépassé. 🎓 La leçon : monte tes niveaux/stats, ou joue les attaques prioritaires pour le devancer.`
            : `Il a placé son burst au bon moment. 🎓 La leçon : garde un Daemon frais en réserve et soigne AVANT le money-time.`
}

function analyze(playerTeam: BattleMon[], enemyTeam: BattleMon[], won: boolean): Recap {
    ...
    const enemyBest = byDmgDesc(enemyTeam)[0] ?? enemyTeam[0]
    const enemyWorst = [...enemyTeam].sort((x, y) => (x.battleBestDmg ?? 0) - (y.battleBestDmg ?? 0))[0] ?? enemyTeam[0]
    ...
    const mvpLine = `A porté l'équipe : ${mvp.battleBestDmgMove ?? "coup signature"} à ${mvp.battleBestDmg ?? 0} dégâts.`
    ...
    if (won) {
        ...
        const top3 = [
            ...
            `${nameOf(enemyWorst)} (adverse) complètement aux fraises — son meilleur coup : ${enemyWorst.battleBestDmg ?? 0} dégâts 🤡`,
        ]
        return {
            intro, top3, mvp, mvpLine,
            foe: enemyWorst, foeTag: "🤡 PAILLASSON",
            foeLine: "Un sweeper qui rate son moment, c'est comme des pâtes sans sel.",
            what: `${decMine ? "Ton" : "Le"} ${nameOf(decisive)} envoie ${decisive.battleBestDmgMove ?? "son attaque la plus lourde"} (${decisive.battleBestDmg ?? "?"} dégâts).`,
            why: lesson(decisive, enemyTeam[0], true), whyLabel: "Pourquoi ça win",
        }
    }

    // DÉFAITE — même richesse, ton "on se relève". Le 2e award devient le BOURREAU (qui t'a démonté).
    const intro =
        enemyDown === 0
            ? "✦ Aïe… ROULÉ dans la semoule, pas une miette grignotée. On se relève et on bosse les contres. ✦"
            : enemyDown >= enemyTeam.length - 1
                ? "✦ Crève-cœur ! Ça s'est joué à UN cheveu de nouille… la revanche est à toi. ✦"
                : "✦ Défaite, mais pas une déroute : t'as mordu, t'as fait mal. Affûte ta tactique et reviens. ✦"
    const top3 = [
        `${nameOf(enemyBest)} adverse t'a démonté avec ${enemyBest.battleBestDmgMove ?? "un gros coup"} (${enemyBest.battleBestDmg ?? "?"} dégâts) 💀`,
        myDown > 0 ? `${myDown} de tes Daemon${myDown > 1 ? "s" : ""} au tapis ⚰️` : `Tu as tenu, mais pas assez longtemps 😤`,
        (mvp.battleBestDmg ?? 0) > 0 ? `${displayName(mvp)} a sauvé l'honneur : ${mvp.battleBestDmg} dégâts au compteur ✊` : `Personne n'a vraiment pris feu de ton côté…`,
    ]
    return {
        intro, top3, mvp,
        mvpLine: (mvp.battleBestDmg ?? 0) > 0 ? `Ton meilleur malgré la défaite : ${mvp.battleBestDmgMove ?? "son coup"} à ${mvp.battleBestDmg} dégâts.` : "Pas son meilleur jour…",
        foe: enemyBest, foeTag: "💀 BOURREAU",
        foeLine: `T'a puni : ${enemyBest.battleBestDmgMove ?? "gros coup"} · ${enemyBest.battleBestDmg ?? 0} dégâts.`,
        what: `Le ${nameOf(enemyBest)} adverse t'a planté ${enemyBest.battleBestDmgMove ?? "son attaque la plus lourde"} (${enemyBest.battleBestDmg ?? "?"} dégâts).`,
        why: lesson(enemyBest, playerTeam[0], false), whyLabel: "Pourquoi ça casse",
    }
}
...
const S: Record<string, React.CSSProperties> = {
    ...
    haloBourreau: { filter: "drop-shadow(0 0 9px #e04040) drop-shadow(0 0 17px #ff5050aa)" }, // BOURREAU (défaite) : rouge menaçant, pas grisé
