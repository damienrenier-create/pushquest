...
import { CASINO_NUM_CASES, CASINO_MIN_BET, CASINO_MAX_BET, CASINO_WIN_MULT, TONYTONY_TARGET } from "@/lib/gamebook/yellow/data/labDefis"
...
export default function CasinoYellowModal({ onClose }: { onClose: () => void }) {
    ...
    const canSpin = !bankruptActive && placed.length > 0 && totalBet > 0 && totalBet <= player.reps
    ...
    const spin = () => {
        ...
        const r = casinoSpin(payload, Date.now())
        if (!r.ok) { setError(r.reason ?? "Erreur"); return }
        setResult(r); setError(null); setBets({})
        persistYellowSave() // persiste tout de suite (cumul + état casino) — anti-perte au refresh
    }
                        ...
                        <>
                            <div style={{ display: "grid", gridTemplateColumns: "repeat(6, 1fr)", gap: 4, marginBottom: 8 }}>
                                {Array.from({ length: CASINO_NUM_CASES }, (_, i) => (
                                    <button key={i} onClick={() => toggle(i)} style={{ ...caseBtn, ...(bets[i] !== undefined ? caseOn : {}) }}>{i}</button>
                                ))}
                            </div>
                            {placed.length > 0 && (
                                <div style={{ background: "#fff8e8", padding: 8, borderRadius: 4, marginBottom: 8, border: `1px solid ${DARK}` }}>
                                    {placed.map((c) => (
                                        <div key={c} style={{ display: "flex", alignItems: "center", gap: 6, marginBottom: 4, fontSize: 11 }}>
                                            ...
                                            <input type="range" min={CASINO_MIN_BET} max={CASINO_MAX_BET} step={1} value={bets[c]} onChange={(e) => setAmt(c, Number(e.target.value))} style={{ flex: 1 }} />
                                            <span style={{ width: 34, textAlign: "right" }}>{bets[c]} ⚡</span>
                                            <button onClick={() => toggle(c)} style={xBtn}>✕</button>
                                        </div>
                                    ))}
                                    <div style={{ marginTop: 4, fontSize: 11, textAlign: "right", color: totalBet > player.reps ? "#c0392b" : INK }}>Total : <b>{totalBet} ⚡</b></div>
                                </div>
                            )}
                            <button onClick={spin} disabled={!canSpin} style={{ ...spinBtn, ...(canSpin ? {} : spinOff) }}>🎰 SPIN</button>
                        </>
                    )}
...
const caseBtn: React.CSSProperties = { background: "#fff8e8", border: `1px solid ${DARK}`, color: INK, padding: "8px 4px", fontSize: 14, fontWeight: 800, cursor: "pointer", borderRadius: 3 }
