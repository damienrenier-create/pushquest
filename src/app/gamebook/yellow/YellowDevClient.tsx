...
export default function YellowDevClient({ userId = "", isCreator = false, nickname = "", gameMode = "normal" }: { userId?: string; isCreator?: boolean; nickname?: string; gameMode?: string }) {
    ...
    const artisaneOpen = useGameStore((s) => s.artisaneOpen)
    const closeArtisane = useGameStore((s) => s.closeArtisane)
    ...
    const { mode: arenaMode, opponents: arenaOpponents } = usePlayerArena(mapPlayer.mapId, player.badges, userId, myArenaLevel, gameMode, player.defeatedTrainers.includes(AUTEL_VISITED_MARKER))
    ...
    const handleArenaClick = (uid: string, skipClanGuard = false) => {
        ...
        if (uid === ARCHIVISTE_ID) {
            ...
            if (!npc) return
            // 1re RENCONTRE : il se présente + REMET le dex (consultable de suite). Pas de combat cette fois — il faudra
            //   revenir le défier pour débloquer les fiches complètes. Le fun fact, lui, arrive à SA DÉFAITE.
            if (!getPlayer().collectionneurDexGiven) {
                setCollectionneurDexGiven(); persistYellowSave()
                showDialogue("archiviste", ARCHIVISTE_NAME, ARCHIVISTE_INTRO_LINES)
                return
            }
            // MAX 5 matchs/jour (gagnés OU perdus) → au-delà, il s'incline et propose de revenir demain.
            ...
            // ESCALADE du jour sur les VICTOIRES uniquement (perdre laisse retenter à difficulté égale) :
            //   0 victoire = base, puis +3 niv/Daemon et +45 pts Saiyan par victoire (1 → +3/50 · 4 → +12/185).
            const { levelBonus, saiyanPoints } = archivisteEscalation(archivisteWinsToday())
    ...
    useEffect(() => {
        ...
        if (duelResult.won) {
            ...
            recordTeamCompoAchievements(getPlayer().team) // HAUTS FAITS mono-stade + mono-type (victoire miroir)
    ...
    const goBack = (): boolean => {
        ...
        if (artisaneOpen) { closeArtisane(); return true }
    ...
    return (
        <div style={pageStyle}>
            ...
            {menu === "pause" && (
                <div style={menuOverlayStyle} onClick={() => { if (Date.now() - menuTapGuard.current < 350) return; setMenu("none") }}>
                    <div style={menuBoxStyle} onClick={(e) => e.stopPropagation()}>
                        ...
                        {/* DEX NEXUS UNIFIÉ (ex Pokédex + Dex-catalogue) : toujours accessible. Révélation à vie + outils libres
                            (recherche, filtre, table des types GLOBALE). CAPTURE = biologie ; L'Archiviste = dossier de combat
                            (stats, forces & faiblesses par espèce, attaques, évolution). */}
            ...
            {!battle && fusionMenuOpen && (() => {
                ...
                const launch = () => {
                    if (!ready) return
                    const pairErr = fusionPairError(picks[0].speciesId, picks[1].speciesId) // même espèce · stade ultime (MégamonarX) · super-fusion = fusion+fusion uniquement
                    if (pairErr) { setToast(pairErr); return }
                    ...
                    <div style={menuOverlayStyle} onClick={closeIt}>
                        <div style={menuBoxStyle} onClick={(e) => e.stopPropagation()}>
                            ...
                            {player.team.map((m) => {
                                ...
                                const picked = idx >= 0
                                // GRISAGE : 2 pris max · stade ultime (MégamonarX) jamais fusionnable · une fois le 1er choisi, on grise
                                //   tout parent INCOMPATIBLE (même espèce, ou fusion↔normal interdit) via la règle partagée fusionPairError.
                                const firstPick = fusionPick.length === 1 ? picks[0] : undefined
                                const dis = !picked && (fusionPick.length >= 2 || NON_FUSABLE_IDS.includes(m.speciesId) || (!!firstPick && fusionPairError(firstPick.speciesId, m.speciesId) !== null))
                                return (
            ...
            {!battle && fusionAtelierOpen && (() => {
                ...
                const confirmAdd = () => {
                    ...
                    // RÈGLES FUSION (partagées, même verdict qu'à l'Autel) : même espèce · stade ultime (MégamonarX) · super-fusion = fusion+fusion only.
                    const sameA = byUid(add.a), sameB = byUid(add.b)
                    if (sameA && sameB) { const err = fusionPairError(sameA.speciesId, sameB.speciesId); if (err) { setToast(err); return } }
                                        ...
                                        return (
                                            <div key={i} style={{ padding: "6px 9px", border: "1px solid #7c4fc0", borderRadius: 8, margin: "4px 0", background: "rgba(124,79,192,0.07)" }}>
                                                <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
                                                    ...
                                                    {a && b && <button title="Voir la fiche complète (stats, attaques, parents)" onClick={() => setFusionDetail({ aId: a.speciesId, bId: b.speciesId })}
                                                        style={{ background: "transparent", border: "1px solid #7c4fc0", color: "#c79cff", borderRadius: 4, cursor: "pointer", fontSize: 11, padding: "1px 6px", flexShrink: 0 }}>📖</button>}
                                                    ...
                                                    <button onClick={() => removeAt(i)} title="Retirer cette fusion" style={{ background: "transparent", border: "1px solid #c83030", color: "#c83030", borderRadius: 4, cursor: "pointer", fontSize: 11, padding: "1px 6px", flexShrink: 0 }}>✕</button>
                                                </div>
            ...
            {archivisteFight && !battle && (
                <ArenaChallengeModal
                    ...
                    onFight={() => {
                        // On CONSOMME le match du jour ICI (vrai lancement) → scouter puis Annuler ne compte pas. Max 5/jour.
