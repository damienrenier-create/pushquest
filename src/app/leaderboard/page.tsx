...
export default async function LeaderboardPage({
    ...
    return (
        <div className="max-w-4xl mx-auto p-2 sm:p-4 space-y-6 sm:space-y-12 pb-20">
            <header className="text-center space-y-3 sm:space-y-4">
                <h1 className="text-2xl sm:text-4xl md:text-6xl font-black text-white tracking-tighter italic uppercase">
                ...
                <table className="w-full text-left border-collapse">
                    <thead>
                        <tr className="bg-slate-800/50">
                            <th className="px-3 sm:px-6 py-4 text-[9px] sm:text-xs font-black uppercase tracking-widest text-slate-400">Rang</th>
                            <th className="px-3 sm:px-6 py-4 text-[9px] sm:text-xs font-black uppercase tracking-widest text-slate-400">Soldat</th>
                            <th className="px-3 sm:px-6 py-4 text-[9px] sm:text-xs font-black uppercase tracking-widest text-slate-400 text-right">Volume</th>
                        </tr>
                    </thead>
                                    ...
                                    <div className="flex flex-col">
                                        <div className="flex items-center gap-1.5 sm:gap-2">
                                            {selectedExercise === 'XP' && entry.level && (
                                                <span className="text-[10px] sm:text-xs font-black text-slate-400 whitespace-nowrap" title={entry.animal}>
                                                    [Lv.{entry.level} {entry.emoji}]
                                                </span>
                                            )}
                                            <Link
                                                href={`/u/${encodeURIComponent(entry.nickname)}`}
                                                className="text-sm sm:text-lg font-bold text-white group-hover:text-indigo-400 transition-colors truncate max-w-[80px] sm:max-w-none"
                                            >
                                                {entry.nickname}
                                            </Link>
                                        </div>
                                        {selectedExercise === 'XP' && entry.xpNextLvl && (
                                            <div className="mt-1 sm:mt-2 w-full max-w-[120px] sm:max-w-[200px]">
                                                <div className="flex justify-between text-[7px] sm:text-[9px] font-bold text-slate-500 uppercase mb-1 whitespace-nowrap gap-2 sm:gap-4">
                                                    <span className="hidden sm:inline">Niveau Suivant</span>
                                                    <span className="inline sm:hidden">Suiv.</span>
                                                    <span className="text-right truncate">Lv.{entry.level + 1} {entry.nextEmoji} ({entry.xpNextLvl} XP)</span>
                                                </div>
                                                <div className="h-1 sm:h-1.5 w-full bg-slate-800 rounded-full overflow-hidden shadow-inner">
