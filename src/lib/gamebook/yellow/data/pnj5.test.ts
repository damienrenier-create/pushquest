...
describe("PNJ 5 — gardien de la Grotte du Nexus (meute des 5 Gek)", () => {
    it("scaling du niveau : base 75, +2/victoire, plafonné à 100", () => {
        expect(pnj5Level(0)).toBe(75)
        expect(pnj5Level(1)).toBe(77)
        expect(pnj5Level(5)).toBe(85)
        expect(pnj5Level(12)).toBe(99)
        expect(pnj5Level(13)).toBe(100) // 75 + 26 = 101 → plafonné
        expect(pnj5Level(50)).toBe(100) // reste plafonné
        expect(PNJ5_BASE_LEVEL).toBe(75)
    })

    it("équipe de 5 Gek, Geaucké en LEAD (team[0]) au bon niveau", () => {
        const team = buildPnj5Team(0)
        ...
        expect(buildPnj5Team(5)[0].level).toBe(85) // niveau scalé
    })
    ...
    it("tous les moveIds de la meute existent réellement (pas de move fantôme)", () => {
        for (const m of buildPnj5Team(0)) {
            for (const slot of m.moves) expect(getMove(slot.moveId), `${m.speciesId}/${slot.moveId}`).toBeTruthy()
        }
    })

    it("le blocage couvre 18-20,33 (et pas la case du gardien 17,33)", () => {
        expect(inPnj5Block(18, 33)).toBe(true)
        expect(inPnj5Block(19, 33)).toBe(true)
