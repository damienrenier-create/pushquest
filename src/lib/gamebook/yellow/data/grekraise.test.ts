...
describe("Grékraise — jumeau ROCHE/FEU de Gékroc (run 2)", () => {
    ...
    it("run 2 (ngplus) : buildGekroc(true) = Grékraise (ROCHE/FEU), même niveau, learnsAllCts", () => {
        const g = buildGekroc(true)
        expect(g.speciesId).toBe("grekraise")
        const sp = getSpecies("grekraise")!
