...
export const MOVES: Record<string, MoveData> = {
    ...
    apotheose: { id: "apotheose", name: "Apothéose", type: "NORMAL", power: 85, accuracy: 100, pp: 10, effect: { adaptiveStab: true }, description: "Attaque ULTIME et adaptative : elle prend le TYPE PRIMAIRE de ton Daemon (donc TOUJOURS le STAB ×1.5) et frappe sur ta MEILLEURE stat offensive (Attaque ou Spécial). Un Feu costaud en Attaque tape enfin du Feu… au physique. Sur une fusion, c'est le type hérité du parent TÊTE." },
    ...
    atrophie: { id: "atrophie", name: "Atrophie", type: "POISON", power: 0, accuracy: 0, pp: 10, priority: 1, costPower: 40, effect: { statChanges: [{ target: "self", stat: "atk", stages: -2 }, { target: "target", stat: "atk", stages: -2 }] }, description: "Signature du charognard : un relent fétide fait fondre les muscles de TOUS les combattants — Attaque divisée par deux (−2, cumulable) pour la cible ET pour le lanceur. Peu lui importe : il ne frappe qu'en spécial. Frappe en PRIORITÉ." },
    ...
    sables_voraces: { id: "sables_voraces", name: "Sables Voraces", type: "SOL", power: 75, accuracy: 100, pp: 10, effect: { drainPct: 50 }, description: "Signature de CROCAVERN : le colosse engloutit sa proie dans des sables mouvants qui la broient — et lui rendent des forces (rend 50% des dégâts infligés en PV, avec son Attaque)." },
    fracas_colosse: { id: "fracas_colosse", name: "Fracas du Colosse", type: "SOL", power: 105, accuracy: 95, pp: 10, effect: { chance: 25, randomStatDrop: ["atk", "def", "spe", "spc"] }, description: "Signature de CROCAVERN : le titan s'abat de tout son poids. L'onde de choc peut fissurer AU HASARD une caractéristique de la cible — Attaque, Défense, Vitesse ou Spéciale (25%)." },
    ...
    // FRAPPE CAVITATION — signature d'OSQUILLE. 1ʳᵉ attaque EAU PHYSIQUE (category forcée) ET 1ʳᵉ priorité EAU du jeu.
    frappe_cavitation: { id: "frappe_cavitation", name: "Frappe Cavitation", type: "EAU", category: "PHYSICAL", power: 55, accuracy: 100, pp: 15, priority: 1, description: "Une bulle de cavitation implose au contact : STAB Eau PHYSIQUE (frappe sur l'Attaque) qui jaillit TOUJOURS en premier (priorité)." },
    ...
    // → Onde Obscure (65, fiable, peut apeurer) → Dévoreur d'Ombres (90/95%, signature : draine 50% mais peut rater — dégâts attendus ≈ 85, aligné sur le top-drain sans l'outlier 100%).
    ...
    devoreur_ombres: { id: "devoreur_ombres", name: "Dévoreur d'Ombres", type: "TENEBRES", power: 90, accuracy: 95, pp: 10, effect: { drainPct: 50 }, description: "Le prédateur dévore l'ombre de sa proie : gros dégâts Ténèbres et récupère la moitié des dégâts infligés — mais l'assaut vorace peut manquer. Signature ténébreuse." },
    ...
    // === CT DU MAÎTRE (Dôme / Usine / Tour) — 4 moves PUISSANTS pour des types SOUS-SERVIS. Récompense de champion : on
    //     en choisit 1 par facilité conquise, jamais 2× le même → triple-champion = 3/4. Débuff GARANTI (coup offensif). ===
    pique_voile: { id: "pique_voile", name: "Piqué Voilé", type: "VOL", power: 110, accuracy: 95, pp: 5, effect: { statChanges: [{ target: "target", stat: "acc", stages: -1 }] }, description: "Un piqué si fulgurant qu'il désoriente la cible : énormes dégâts VOL et baisse à COUP SÛR sa Précision (-1). CT du Maître." },
    hypercut: { id: "hypercut", name: "Hypercut", type: "COMBAT", power: 110, accuracy: 95, pp: 5, effect: { statChanges: [{ target: "target", stat: "def", stages: -1 }] }, description: "Un uppercut titanesque qui fend la garde : énormes dégâts COMBAT et baisse à COUP SÛR la Défense de la cible (-1). CT du Maître." },
    // ── CT DES 3 CLANS (Chapelle de Nouillon) — signatures uniques, introuvables ailleurs ──
    oeil_du_cyclone: { id: "oeil_du_cyclone", name: "Œil du Cyclone", type: "VOL", power: 30, accuracy: 100, pp: 15, effect: { statChanges: [{ target: "self", stat: "spe", stages: 1 }, { target: "self", stat: "eva", stages: 1 }] }, description: "Le voltigeur s'élève dans l'œil de la tempête : légère frappe, puis il gagne en Vitesse (+1) ET en Esquive (+1). Setup de balayage aérien. CT du clan de l'Air." },
    impact: { id: "impact", name: "Impact", type: "COMBAT", power: 70, accuracy: 90, pp: 10, effect: { oneOf: [{ flinch: true }, { inflictStatus: "PARALYSIS" }, { statChanges: [{ target: "self", stat: "atk", stages: 1 }] }] }, description: "Un choc brutal aux conséquences imprévisibles : à l'impact, UN effet au hasard — soit APEURER la cible, soit la PARALYSER, soit galvaniser le lanceur (Attaque +1). CT du clan du Combat." },
    cristallisation: { id: "cristallisation", name: "Cristallisation", type: "ROCHE", power: 0, accuracy: 80, pp: 10, costPower: 40, effect: { selfVolatile: "CRYSTAL" }, description: "Le colosse se pare d'une carapace de cristal : la PROCHAINE attaque de dégâts encaissée est absorbée et convertie en SOIN (l'éventuel statut est quand même subi). CT du clan de la Roche." },
    transcendance: { id: "transcendance", name: "Transcendance", type: "COMBAT", category: "PHYSICAL", displayType: "Transcendance", power: 100, accuracy: 100, pp: 5, effect: { noStab: true, effectivenessOverride: { VOL: 2, ROCHE: 2, COMBAT: 2, PLANTE: 0.5, INSECTE: 0.5, GLACE: 0.5, NORMAL: 0.5, METAL: 0.5, TENEBRES: 0.5 } }, description: "L'attaque TRANSCENDANTE : un type à part, super efficace (×2) contre Vol, Roche ET Combat, mais peu efficace (×0.5) contre Plante, Insecte, Glace, Normal, Métal et Ténèbres. Jamais de STAB. CT ultime des clans (Daemon-clan niv 80, une seule fois)." },
    vif_dard: { id: "vif_dard", name: "Vif-Dard", type: "INSECTE", power: 90, accuracy: 100, pp: 5, priority: 1, effect: { statChanges: [{ target: "target", stat: "spc", stages: -1 }] }, description: "Un dard fulgurant qui frappe TOUJOURS en premier (priorité) et perce la carapace mentale : gros dégâts INSECTE et baisse à COUP SÛR le Spécial de la cible (-1). CT du Maître." },
    poing_fusion: { id: "poing_fusion", name: "Poing Fusion", type: "METAL", power: 100, accuracy: 95, pp: 5, effect: { chance: 50, inflictStatus: "BURN" }, description: "Un poing d'acier chauffé à blanc : gros dégâts MÉTAL et 50 % de BRÛLER la cible. CT du Maître." },
    // ── CT STATUT INÉDITES (1 par type sans statut : Feu/Glace/Insecte/Ténèbres/Métal/Fée) — boutique de JETONS ──
    fournaise: { id: "fournaise", name: "Fournaise", type: "FEU", power: 0, accuracy: 85, pp: 10, costPower: 60, effect: { inflictStatus: "BURN" }, description: "Enveloppe la cible d'un souffle ardent : la BRÛLE (halve l'Attaque + dégâts par tour). La seule brûlure-statut pure du type Feu." },
    etreinte_glaciale: { id: "etreinte_glaciale", name: "Étreinte Glaciale", type: "GLACE", power: 0, accuracy: 100, pp: 15, costPower: 50, effect: { statChanges: [{ target: "target", stat: "spe", stages: -1 }, { target: "target", stat: "def", stages: -1 }] }, description: "Le gel enserre la cible : -1 Vitesse ET -1 Défense. L'engourdit sans la figer complètement." },
    fil_entravant: { id: "fil_entravant", name: "Fil Entravant", type: "INSECTE", power: 0, accuracy: 100, pp: 20, costPower: 40, effect: { statChanges: [{ target: "target", stat: "spe", stages: -1 }, { target: "target", stat: "acc", stages: -1 }] }, description: "Une toile poisseuse ralentit et aveugle la cible : -1 Vitesse ET -1 Précision." },
