...
interface BattleStoreState {
    ...
    /** DÉFI NÉMÉSIS — message de défaite (« LE NÉMÉSIS ») → Caninombre scellé à jamais. Optionnel (câblage minimal). */
    nemesisLossTaunt?: string | null
...
function persistFusionGauntletWear(finalTeam: ReadonlyArray<BattleMon>): void {
    ...
    const json = serializeGauntletCarryJson({ platine: snapshotPlatineRun() })
...
function finishBattle(b: BattleState, newDexEntry: BattleStoreState["newDexEntry"] = null) {
    ...
    if (b.outcome === "win" && storeState.trainer) {
        ...
        if (storeState.trainer.trainerId?.startsWith("collectionneur:")) {
            ...
            const nm = pick ? (pick.nickname || getSpecies(pick.speciesId)?.name || pick.speciesId) : "champion"
            const fact = pick ? (funFactFor(pick.speciesId) ?? null) : null
            rematchReward = { npcId: "archiviste", npcName: "L'Archiviste", lines: archivisteDefeatLines(nm, fact, archivisteMatchesToday()) }
        }
...
export function clearWhiteout() {
    setStore({ ...storeState, whiteout: false, aceLossTaunt: null, nemesisLossTaunt: null })
}
...
/** DÉFI NÉMÉSIS — message de défaite (lu dans l'effet whiteout) ; null si la défaite n'est pas contre le némésis. */
export function getNemesisLossTaunt(): string | null {
    return storeState.nemesisLossTaunt ?? null
}
...
export function startPvpBattle(battle: BattleState, ctx: Omit<PvpContext, "seq" | "myAction" | "oppAction" | "won" | "desync">) {
    ...
    for (const m of battle.player.team) { m.heldItem = undefined; m.heldItem2 = undefined }
    for (const m of battle.enemy.team) { m.heldItem = undefined; m.heldItem2 = undefined }
