...
interface PlayerState {
    ...
    /** L'ARCHIVISTE : le dex-catalogue a-t-il été OFFERT (1re victoire) ? Effectivement global (OR-mergé au load). */
    ...
    orcalineDate: string
    /** PROF. CHEN (mode FUN) : nb de cadeaux d'énergie de bienvenue déjà réclamés (0..2). */
    chenGiftClaims: number
    ...
    /** ARC LAMPE & GÉNIE — colporteur déjà rencontré/battu ? GLOBAL au profil (1 génie/profil, pas par run). */
    genieArcSeen?: boolean
    /** MODE GENÈSE — profil « 6 craftés / zéro capture jusqu'à la Ligue de Fusion ». GLOBAL au profil. */
    genesisMode?: boolean
    ...
    seenThisRun: string[]
    /** L'ARCHIVISTE — DOSSIER DE COMBAT (stats, forces/faiblesses, attaques, évolution) débloqué CE run (per-monde),
     *  rempli en bloc à SA DÉFAITE. (Dex FUSIONNÉ : voir = sprite+types ; capturer = biologie/dicton/mensurations.) */
    ...
    /** CLANS — registre des clans DÉJÀ REJOINTS sur la partie EN COURS (run 1 → NG+ → run 3). Sert au gate FUN des lignées
     *  « meme » (plumiot/maitrezenc/mottoche ne poppent que si le clan du type a été rejoint). Recopié par startNgPlusWorld/
     *  startRun3World → survit d'un run au suivant ; REMIS À ZÉRO par un restart complet (resetForIntro). Additif → save-safe. */
    clansEverJoined?: ClanKey[]
...
//  - "fun"      : lien "amis" (nexus-fun-2026). Don de DÉPART unique = 10000⚡ + 10 Nexus-Ball (gros pécule
//                 pour vivre la découverte sans se soucier de l'énergie), PUIS l'énergie fonctionne comme en
//                 normal (les vrais reps encodés comptent, cf. bankReps). Interface muscu simplifiée (cf.
//                 SimpleRepsDashboard). PAS de système de remplissages (fill=0).
...
/** Mode fun : pécule de bienvenue UNIQUE (⚡ + relève le plafond d'autant). Généreux pour vivre la DÉCOUVERTE
 *  sans se soucier de l'énergie. Voir ensureModeStartGrant (crédité une seule fois, monde live). */
const FUN_START_GRANT = 10000
 ...
 *  remplissage ; fun : FUN_START_GRANT⚡ + 10 Nexus-Ball), une seule fois (idempotent via modeFillsUsed>0, monde live).
 *  No-op en normal ou si déjà démarré. */
export function ensureModeStartGrant(): void {
    // FUN : don de bienvenue unique = FUN_START_GRANT⚡ + 10 Nexus-Ball. Ensuite l'énergie vient des vrais reps (bankReps)
...
export function hydratePlayer(p: Partial<PlayerState>) {
    st = {
        ...
        collectionneurDexGiven: p.collectionneurDexGiven ?? st.collectionneurDexGiven ?? false,
        ...
        chenGiftClaims: p.chenGiftClaims ?? st.chenGiftClaims ?? 0,
        ...
        // Champs GLOBAUX au profil, EFFAÇABLES pour un profil neuf → pattern `in` (hydrateFromWorld passe toujours la clé,
        //   valeur undefined pour un emptySave → efface au lieu de fuiter l'ancien profil via `?? st`).
        ngplusStarterBase: "ngplusStarterBase" in p ? p.ngplusStarterBase : st.ngplusStarterBase,
        genieArcSeen: "genieArcSeen" in p ? p.genieArcSeen : st.genieArcSeen,
        genesisMode: "genesisMode" in p ? p.genesisMode : st.genesisMode,
        ...
        // GLOBAL au profil, EFFAÇABLE pour un profil neuf → pattern `in` (évite de fuiter les clans d'un ancien profil).
        clansEverJoined: "clansEverJoined" in p ? p.clansEverJoined : st.clansEverJoined,
...
export function markCaughtThisRun(speciesId: string) {
    markSeenThisRun(speciesId) // capturé/offert/starter ⇒ VU ce run (LIGNE du dex de L'Archiviste)
    if (!speciesId || st.caughtThisRun.includes(speciesId)) return
    st = { ...st, caughtThisRun: [...st.caughtThisRun, speciesId] }
    emit()
}

/** L'ARCHIVISTE — marque une espèce comme VUE CE run (LIGNE du dex, overlay per-monde). Idempotent. Appelé en
 *  paire de chaque markSeen (pokedexStore) → un joueur run 1 ne voit QUE des lignes run 1 (scoping strict natif). */
export function markSeenThisRun(speciesId: string) {
    if (!speciesId || st.seenThisRun.includes(speciesId)) return
    st = { ...st, seenThisRun: [...st.seenThisRun, speciesId] }
    emit()
}

/** L'ARCHIVISTE — MIGRATION (one-time) : pour un ancien save (seenThisRun absent/vide), on SEED les lignes depuis
 *  les espèces déjà vues/possédées → aucune perte de progression. Ne fait rien si des lignes existent déjà. */
export function seedSeenThisRun(ids: readonly string[]) {
    if (st.seenThisRun.length > 0) return
    const uniq = [...new Set(ids.filter((id) => !!id))]
    if (uniq.length === 0) return
    st = { ...st, seenThisRun: uniq }
    emit()
}

/** L'ARCHIVISTE — DÉFAITE : débloque EN BLOC les fiches de toutes les lignes actuellement vues (seenThisRun).
 *  Les rencontres faites APRÈS restent verrouillées jusqu'à la prochaine victoire. Idempotent. */
export function unlockFichesFromSeen() {
    const merged = [...new Set([...st.fichesUnlockedThisRun, ...st.seenThisRun])]
    if (merged.length === st.fichesUnlockedThisRun.length) return
    st = { ...st, fichesUnlockedThisRun: merged }
    emit()
}

/** L'ARCHIVISTE — la fiche de cette espèce est-elle consultable CE run ? */
export function isFicheUnlocked(speciesId: string): boolean { return st.fichesUnlockedThisRun.includes(speciesId) }

/** L'ARCHIVISTE — offre le dex-catalogue (1re victoire). Effectivement global (OR-mergé au load). Idempotent. */
export function setCollectionneurDexGiven() {
    if (st.collectionneurDexGiven) return
    st = { ...st, collectionneurDexGiven: true }
    emit()
}
...
export function startNgPlusWorld(starter: MonInstance) {
    st = { team: [starter], pc: [], items: {}, reps: 0, repsCap: 1000, creditedThrough: "", repsBankedTotal: -1, welcomeGift: true, pokerFirstGameDone: st.pokerFirstGameDone, pokerBossStacks: {}, pokerCashCap: 0, pokerCashDate: "", spagGift: true, pastaGodGift: true, pastaBoughtToday: 0, casinoSpentToday: 0, pastaDayBonus: 0, defeatedTrainers: [], rematchedTrainers: [], badges: [], wildCtx: st.wildCtx, introSeen: true, sbireDefeatsToday: 0, sbireWinsTotal: 0, pvpStats: emptyPvpStats(), stats: emptyYellowStats(), acePeakLevel: 0, aceBox: {}, aceTeamSizePeak: 3, aceWins: 0, aceDefeatedDate: "", duelWins: {}, ownedCts: [], boughtCts: [], gekrocResolved: false, hhSpectresShown: [], hhCollectorWins: 0, isChampion: false, leagueSixShiny: false, mirrorWinHigherLevel: false, berrySecretKnown: false, collectionneurDexGiven: false, berryHarvestDay: "", berryHarvestPicked: [], caveTradeDone: false, goshHintHeard: false, orcalineWins: 0, orcalineDate: "", chenGiftClaims: 0, pnj5Wins: 0, ngplusBattles: 0, moveReminderUses: 0, sylvebarbeAwake: false, labDefi: emptyLabDefi(), funDefis: emptyFunDefis(), customDaemons: st.customDaemons, clansEverJoined: st.clansEverJoined ?? [], ngplusStartedAt: Date.now(), playtimeMs: 0, leaguePotions: 0, ngplusUsed: true, run3Used: false, ngplusMaitreBeaten: false, run3StarterBase: "", ngplusStarterBase: starter.speciesId, genieArcSeen: st.genieArcSeen, genesisMode: st.genesisMode, chosenAvatar: st.chosenAvatar, domeChampionships: st.domeChampionships, run3Defeated: [], run3EnergyByArena: {}, caughtThisRun: [], seenThisRun: [], fichesUnlockedThisRun: [], fusionRoster: [], fusionHistory: [], run3LavapetitSeen: false, run3LavapetitCaught: false, mimimoyReturned: false, mimimoyAppearances: 0, ballLockRemaining: 0, luxeOutcomeQueue: [] }
    emit()
}
...
export function startRun3World(starter: MonInstance) {
    st = { team: [starter], pc: [], items: {}, reps: 0, repsCap: 1000, creditedThrough: "", repsBankedTotal: -1, welcomeGift: true, pokerFirstGameDone: st.pokerFirstGameDone, pokerBossStacks: {}, pokerCashCap: 0, pokerCashDate: "", spagGift: true, pastaGodGift: true, pastaBoughtToday: 0, casinoSpentToday: 0, pastaDayBonus: 0, defeatedTrainers: [], rematchedTrainers: [], badges: [], wildCtx: st.wildCtx, introSeen: true, sbireDefeatsToday: 0, sbireWinsTotal: 0, pvpStats: emptyPvpStats(), stats: emptyYellowStats(), acePeakLevel: 0, aceBox: {}, aceTeamSizePeak: 3, aceWins: 0, aceDefeatedDate: "", duelWins: {}, ownedCts: [], boughtCts: [], gekrocResolved: false, hhSpectresShown: [], hhCollectorWins: 0, isChampion: false, leagueSixShiny: false, mirrorWinHigherLevel: false, berrySecretKnown: false, collectionneurDexGiven: false, berryHarvestDay: "", berryHarvestPicked: [], caveTradeDone: false, goshHintHeard: false, orcalineWins: 0, orcalineDate: "", chenGiftClaims: 0, pnj5Wins: 0, ngplusBattles: 0, moveReminderUses: 0, sylvebarbeAwake: false, labDefi: emptyLabDefi(), funDefis: emptyFunDefis(), customDaemons: st.customDaemons, clansEverJoined: st.clansEverJoined ?? [], ngplusStartedAt: Date.now(), playtimeMs: 0, leaguePotions: 0, ngplusUsed: true, run3Used: true, ngplusMaitreBeaten: false, domeChampionships: st.domeChampionships, run3StarterBase: starter.speciesId, ngplusStarterBase: st.ngplusStarterBase, genieArcSeen: st.genieArcSeen, genesisMode: st.genesisMode, chosenAvatar: st.chosenAvatar, run3Defeated: [], run3EnergyByArena: {}, caughtThisRun: [], seenThisRun: [], fichesUnlockedThisRun: [], fusionRoster: [], fusionHistory: [], run3LavapetitSeen: false, run3LavapetitCaught: false, mimimoyReturned: false, mimimoyAppearances: 0, ballLockRemaining: 0, luxeOutcomeQueue: [] }
    emit()
}
...
// ════════════ VŒU DU GÉNIE — CAP CASINO (mise ≤ 250 ⚡ + 250 ⚡/jour) ════════════
...
// MALÉDICTION ASSOUPLIE (Sartay, 25/08/2026) : le « banni du casino » (ex-vœu de Jacanon) peut désormais miser
//   jusqu'à 250 ⚡/JOUR — mise unique ≤ 250, total quotidien ≤ 250 (reset via creditDailyReps). Remplace l'ancien
//   régime dur (10/mise + plafond progressif 50→1000) par un plafond FLAT, INDÉPENDANT de l'historique de la save
//   → Jacanon retrouve ses 250/jour immédiatement, quel que soit son ancien casinoCapToday (champ conservé mais inutilisé).
export const CASINO_DAILY_CAP = 250
export const CASINO_VOW_MAX_BET = 250
/** Plafond de mise QUOTIDIEN d'un joueur restreint (flat 250 ⚡/jour). Infini si non restreint. */
export function casinoDailyCap(): number {
    return isCasinoRestricted() ? CASINO_DAILY_CAP : Infinity
}
...
export function casinoBetAllowed(bet: number): { ok: boolean; reason?: string } {
    ...
    if (st.casinoSpentToday + b > cap) return { ok: false, reason: `Vœu du génie : plafond de ${cap} ⚡/jour au casino atteint. Reviens demain.` }
    return { ok: true }
}
...
/** Clans DÉJÀ REJOINTS À VIE (tous runs) = registre persistant ∪ clan du run EN COURS (couvre les saves legacy sans registre,
 *  et le clan fraîchement pacté avant même le forward-copy). Sert au gate FUN des lignées « meme ». */
export function getClansEverJoined(): ClanKey[] {
    const ever = st.clansEverJoined ?? []
    const cur = st.clan
    return cur && !ever.includes(cur) ? [...ever, cur] : [...ever]
}
/** Prête serment (IRRÉVERSIBLE ce run) : mémorise le clan + l'uid du Daemon-clan offert, et l'inscrit au registre À VIE
 *  (sauf REJEU jetable). Ce registre débloque la pop des lignées « meme » du type sur TOUS les runs (mode fun). */
export function joinClan(clan: ClanKey, daemonUid: string) {
    const ever = st.clansEverJoined ?? []
    const nextEver = (activeWorld === "replay" || ever.includes(clan)) ? ever : [...ever, clan]
    st = { ...st, clan, clanDaemonUid: daemonUid, clansEverJoined: nextEver }
    emit()
}
...
/** FASHION VICTIM — pose (ou retire) l'avatar choisi (chemin de planche Gen3). Validé par l'appelant (isValidAvatar) ; persisté via persistYellowSave. */
export function setChosenAvatar(path: string | undefined) {
    st = { ...st, chosenAvatar: path }
    emit()
}
...
// ═══════════════════════ PÂTE DE LUXE (loterie génétique) ═══════════════════════
/** Issues possibles d'une Pâte de Luxe. "perfect" = IV max · "min" = IV 0 (rang D) · "shiny_perfect" = shiny + IV max. */
export type LuxeOutcome = "perfect" | "min" | "shiny_perfect"
const PERFECT_IVS = { hp: IV_MAX, atk: IV_MAX, def: IV_MAX, spe: IV_MAX, spc: IV_MAX }
...
function applyLuxeOutcome(mon: MonInstance, outcome: LuxeOutcome): MonInstance {
    ...
    if (sp) next.currentHp = mon.currentHp <= 0 ? 0 : fullStats(next, sp).hp // un K.O. reste K.O. ; sinon PV pleins au nouveau max
    return next
}
 ...
 *  1 PARFAIT, 1 SHINY+PARFAIT, 1 rang D, et le reste en 50/50 — surprise à l'usage (ordre aléatoire). */
export function grantLuxePastaBatch(n = 6): void {
    const outcomes: LuxeOutcome[] = []
    for (const g of ["perfect", "shiny_perfect", "min"] as LuxeOutcome[]) if (outcomes.length < n) outcomes.push(g)
    while (outcomes.length < n) outcomes.push(Math.random() < 0.5 ? "perfect" : "min")
...
/** Plafond SOUPLE : crédite des reps AU-DESSUS du cap dur mais borné à softMult×repsCap. Le cap N'EST PAS relevé →
 *  l'excédent est toléré puis retombe au cap dur au prochain bankReps quotidien (cadeau « souple/éphémère », ≠
 *  grantBonusEnergyUncapped qui relève le cap durablement). Ne réduit jamais un solde déjà au-dessus du plafond
 *  souple. Renvoie le montant réellement ajouté. Bloqué en run3 (source unique) sauf `force`. */
export function grantRepsSoftCap(n: number, softMult = 2, force = false): number {
    if (runMode() === "run3" && !force) return 0
    const amt = Math.max(0, Math.floor(n))
    if (amt <= 0) return 0
    const soft = Math.floor(st.repsCap * Math.max(1, softMult)) // plafond souple = softMult × cap dur
    const before = st.reps
    const ceiling = Math.max(before, soft) // ne baisse jamais un solde déjà au-dessus du plafond souple
    st = { ...st, reps: Math.min(ceiling, before + amt) }
    emit()
    return st.reps - before
}

/** PROF. CHEN (mode FUN) — cadeaux de bienvenue encore disponibles (0..2). */
export function chenGiftsRemaining(): number { return Math.max(0, 2 - (st.chenGiftClaims ?? 0)) }
/** PROF. CHEN (mode FUN) — réclame UN cadeau (crédit soft-cap) : incrémente le compteur (borné 2), crédite `amount`,
 *  journalise. Renvoie { added, tier } (tier = 1 ou 2 = n° du cadeau réclamé) ou null si épuisé. */
export function claimChenGift(amount: number): { added: number; tier: number } | null {
    if (chenGiftsRemaining() <= 0) return null
    const tier = (st.chenGiftClaims ?? 0) + 1
    const added = grantRepsSoftCap(amount)
    st = { ...st, chenGiftClaims: tier }
...
export function funOnBadge(): number {
    ...
    const avgLvl = st.team.length ? st.team.reduce((s, m) => s + m.level, 0) / st.team.length : 1
    const added = grantReps(funArenaRewardScaled(st.badges.length, avgLvl)) // 100→300 (par n° d'arène) × bonus de niveau (×1→×2)
...
/** ARC LAMPE & GÉNIE — marque le colporteur rencontré/battu (GLOBAL au profil) → le génie n'apparaît qu'1× par
 *  profil (plus à chaque run). Carry across-monde (comme customDaemons) ; frais pour un NOUVEAU profil. */
export function markGenieArcSeen(): void {
    if (st.genieArcSeen) return
    st = { ...st, genieArcSeen: true }
    emit()
}
export function isGenieArcSeen(): boolean { return !!st.genieArcSeen }
/** MODE GENÈSE — profil « 6 Daemons craftés / zéro capture » actif ? */
export function isGenesisMode(): boolean { return !!st.genesisMode }
/** MODE GENÈSE — la CAPTURE est-elle verrouillée ? (mode actif ET Ligue de Fusion pas encore gagnée = fusleague_or). */
export function genesisCaptureLocked(): boolean { return !!st.genesisMode && !st.defeatedTrainers.includes("fusleague_or") }
