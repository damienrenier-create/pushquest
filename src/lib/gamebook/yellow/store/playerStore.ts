...
import { LAMP_ITEM_ID } from "../data/genieLamp"
...
interface PlayerState {
    ...
    /** Compteurs des hauts faits « consolation » (poisse). */
    achTrack: AchTrack
...
export function hydratePlayer(p: Partial<PlayerState>) {
    st = {
        ...
        achTrack: p.achTrack ?? st.achTrack ?? emptyAchTrack(),
...
export function recordCaptureResult(caught: boolean): void {
    const t = st.achTrack
    if (caught) { if (t.ballMiss !== 0) { st = { ...st, achTrack: { ...t, ballMiss: 0 } }; emit() } return }
    const ballMiss = t.ballMiss + 1
    const add = ballMiss >= 10 && !st.defeatedTrainers.includes("ach_ball_miss10") ? ["ach_ball_miss10"] : []
    st = { ...st, achTrack: { ...t, ballMiss }, defeatedTrainers: add.length ? [...st.defeatedTrainers, ...add] : st.defeatedTrainers }
    emit()
}
/** CONSOLATIONS de DÉFAITE : 3 défaites d'affilée vs le MÊME PNJ + 10 défaites/jour. Réinitialise aussi la série « sans KO ». */
export function recordBattleLoss(trainerId: string, today: string): void {
    const t = st.achTrack
    const foeLoss = trainerId && trainerId === t.foeId ? t.foeLoss + 1 : 1
    const lossCount = (t.lossDate === today ? t.lossCount : 0) + 1
    const add: string[] = []
    if (foeLoss >= 3 && !st.defeatedTrainers.includes("ach_samefoe_loss3")) add.push("ach_samefoe_loss3")
    if (lossCount >= 10 && !st.defeatedTrainers.includes("ach_losses10_day")) add.push("ach_losses10_day")
    st = { ...st, achTrack: { ...t, foeId: trainerId || t.foeId, foeLoss, lossDate: today, lossCount, noKoWins: 0 }, defeatedTrainers: add.length ? [...st.defeatedTrainers, ...add] : st.defeatedTrainers }
    emit()
}
/** MAÎTRISE « 10 combats dresseur sans un seul KO subi » : série de victoires 0-KO (remise à zéro dès qu'un KO est subi). */
export function recordTrainerWinNoKo(faintsTaken: number): void {
    const t = st.achTrack
    if (faintsTaken > 0) { if (t.noKoWins !== 0) { st = { ...st, achTrack: { ...t, noKoWins: 0 } }; emit() } return }
    const noKoWins = t.noKoWins + 1
    const add = noKoWins >= 10 && !st.defeatedTrainers.includes("ach_noko_win10") ? ["ach_noko_win10"] : []
    st = { ...st, achTrack: { ...t, noKoWins }, defeatedTrainers: add.length ? [...st.defeatedTrainers, ...add] : st.defeatedTrainers }
    emit()
}
