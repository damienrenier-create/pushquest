...
import { getPlayer, hydratePlayer, subscribePlayer, setWildCtx, creditDailyReps, bankReps, creditReps, logEnergyIncome, restoreRepsState, claimWelcomeGift, claimSpagGift, applySaiyanResults, resetForIntro, reregisterCustomDaemons, getActiveWorld, setActiveWorld, startNgPlusWorld, startRun3World, raiseRepsCap, grantReps, addItem, setBerrySecretKnown, setCollectionneurDexGiven, setBadgeRepsClaimed, seedSeenThisRun, ensureModeStartGrant, startReplayWorld, getReplayRun, getReplayReturn, setReplayContext, clearReplayContext, getGameMode, reconcileCrossRunItems, fixNgplusFusioBalls, openPlatineIfOrCleared } from "./playerStore"
import { getPokedex, hydratePokedex, subscribePokedex } from "./pokedexStore"
import { getSpecies, SPECIES } from "../data/species"
import { parseSave, emptySave, type YellowSave, type ChampionMon, SAVE_VERSION, ENERGY_LOG_MAX, MSG_LOG_MAX } from "../storage/save"
...
function hydrateFromWorld(w: YellowSave, customDaemons: StoredCustomDaemon[]): void {
    hydratePlayer({ team: w.team, pc: w.pc, items: w.items, reps: w.reps, repsCap: w.repsCap, creditedThrough: w.creditedThrough, pastaBoughtToday: w.pastaBoughtToday, casinoSpentToday: w.casinoSpentToday, pastaDayBonus: w.pastaDayBonus, defeatedTrainers: w.defeatedTrainers, rematchedTrainers: w.rematchedTrainers, badges: w.badges as BadgeId[], introSeen: w.introSeen, sbireDefeatsToday: w.sbireDefeatsToday, champBattlesToday: w.champBattlesToday, consultsToday: w.consultsToday, archivisteMatchesToday: w.archivisteMatchesToday, archivisteWinsToday: w.archivisteWinsToday, badgeRepsClaimed: w.badgeRepsClaimed, badgeRepsToday: w.badgeRepsToday, comparisonConsultsToday: w.comparisonConsultsToday, sageSaiyanPointsToday: w.sageSaiyanPointsToday, ananasLastBadgeCount: w.ananasLastBadgeCount, ananasDate: w.ananasDate, ananasPeakLevel: w.ananasPeakLevel, capturesToday: w.capturesToday, sbireWinsTotal: w.sbireWinsTotal, pvpStats: w.pvpStats, domeStats: w.domeStats, domeTierRecord: w.domeTierRecord, stats: w.stats, acePeakLevel: w.acePeakLevel, aceBox: w.aceBox, aceTeamSizePeak: w.aceTeamSizePeak, aceWins: w.aceWins, aceDefeatedDate: w.aceDefeatedDate, duelWins: w.duelWins, ownedCts: w.ownedCts, boughtCts: w.boughtCts, gekrocResolved: w.gekrocResolved, hhSpectresShown: w.hhSpectresShown, hhCollectorWins: w.hhCollectorWins, isChampion: w.isChampion, leagueSixShiny: w.leagueSixShiny, mirrorWinHigherLevel: w.mirrorWinHigherLevel, berrySecretKnown: w.berrySecretKnown, berryHarvestDay: w.berryHarvestDay, berryHarvestPicked: w.berryHarvestPicked, sylvebarbeAwake: w.sylvebarbeAwake, caveTradeDone: w.caveTradeDone, goshHintHeard: w.goshHintHeard, orcalineWins: w.orcalineWins, orcalineDate: w.orcalineDate, chenGiftClaims: w.chenGiftClaims, funDefis: w.funDefis, achTrack: w.achTrack, pnj5Wins: w.pnj5Wins, ngplusBattles: w.ngplusBattles, repsBankedTotal: w.repsBankedTotal, welcomeGift: w.welcomeGift, pokerFirstGameDone: w.pokerFirstGameDone, pokerBossStacks: w.pokerBossStacks, pokerCashCap: w.pokerCashCap, pokerCashDate: w.pokerCashDate, spagGift: w.spagGift, pastaGodGift: w.pastaGodGift, labDefi: w.labDefi, ngplusStartedAt: w.ngplusStartedAt, playtimeMs: w.playtimeMs, leaguePotions: w.leaguePotions, ngplusUsed: w.ngplusUsed, run3Used: w.run3Used, ngplusMaitreBeaten: w.ngplusMaitreBeaten, run3StarterBase: w.run3StarterBase, ngplusStarterBase: w.ngplusStarterBase, genieArcSeen: w.genieArcSeen, genesisMode: w.genesisMode, run3Defeated: w.run3Defeated, run3EnergyByArena: w.run3EnergyByArena, caughtThisRun: w.caughtThisRun, seenThisRun: w.seenThisRun, fichesUnlockedThisRun: w.fichesUnlockedThisRun, collectionneurDexGiven: w.collectionneurDexGiven, fusionRoster: w.fusionRoster, fusionNames: w.fusionNames, fusionMoveOrders: w.fusionMoveOrders, fusionTypeChoices: w.fusionTypeChoices, fusionHistory: w.fusionHistory, energyLog: w.energyLog, messageLog: w.messageLog, run3LavapetitSeen: w.run3LavapetitSeen, run3LavapetitCaught: w.run3LavapetitCaught, mimimoyReturned: w.mimimoyReturned, mimimoyAppearances: w.mimimoyAppearances, ballLockRemaining: w.ballLockRemaining, evResetCharges: w.evResetCharges, shinyWish: w.shinyWish, forcedEncounter: w.forcedEncounter, fusionLeagueCarry: w.fusionLeagueCarry, curseAbundanceStart: w.curseAbundanceStart, curseFreeItemsTaken: w.curseFreeItemsTaken, curseFreeItemDate: w.curseFreeItemDate, potionBuysToday: w.potionBuysToday, jcEnergyBuysToday: w.jcEnergyBuysToday, casinoCapToday: w.casinoCapToday, fusionLeagueDefeats: w.fusionLeagueDefeats, grotteShopBuys: w.grotteShopBuys, chosenAvatar: w.chosenAvatar, clan: w.clan, clanDaemonUid: w.clanDaemonUid, clanCtGiven: w.clanCtGiven, clanTransGiven: w.clanTransGiven, clanVisitDate: w.clanVisitDate, clanTrainByClan: w.clanTrainByClan, craftedItems: w.craftedItems, craftsUsed: w.craftsUsed, craftReady: w.craftReady, megaInLigue: w.megaInLigue, fusionLeagueTryDate: w.fusionLeagueTryDate, fusionChampionRoster: w.fusionChampionRoster, fusionChampionAvatar: w.fusionChampionAvatar, domeChampionships: w.domeChampionships, customDaemons })
...
export function applyServerSave(save: YellowSave): void {
    ...
    // CORRECTIF run 2 : les Fusio-Balls du kit de départ fun (inutiles hors fusion) → Nexus-Balls, au chargement du
    //   monde ngplus (un patch serveur serait clobberé par l'autosave client). Idempotent, re-persisté au prochain save.
    if (aw === "ngplus") activeData.items = fixNgplusFusioBalls(activeData.items ?? {})
    // PALIER PLATINE : quiconque a déjà bouclé l'OR voit la porte du trône s'ouvrir, sans refaire la Ligue.
    activeData.defeatedTrainers = openPlatineIfOrCleared(activeData.defeatedTrainers ?? [])
    ...
    for (const w of pdxWorlds) {
        const sa = w.pokedex?.seenAt
        if (!sa) continue
        for (const [id, zones] of Object.entries(sa)) {
            const set = new Set(mergedSeenAt[id] ?? [])
            for (const z of zones) if (z) set.add(z)
            mergedSeenAt[id] = [...set]
        }
    }
    const mergedFirstCatch: Record<string, { mapId: string; at: string }> = {}
    const considerFirst = (id: string | undefined, mapId: string | undefined, at: string | undefined) => {
        if (!id || !mapId) return
        const atv = at ?? ""
        const cur = mergedFirstCatch[id]
        // garde la date la PLUS ANCIENNE (YYYY-MM-DD comparable ; "" = inconnue → ne remplace jamais une vraie date).
        if (!cur) mergedFirstCatch[id] = { mapId, at: atv }
        else if (atv && (!cur.at || atv < cur.at)) mergedFirstCatch[id] = { mapId, at: atv }
    }
    for (const w of pdxWorlds) { const fc = w.pokedex?.firstCatch; if (fc) for (const [id, e] of Object.entries(fc)) considerFirst(id, e?.mapId, e?.at) }
    ...
    if (pdxWorlds.some((w) => w.collectionneurDexGiven)) setCollectionneurDexGiven()
    setBadgeRepsClaimed(pdxWorlds.flatMap((w) => w.badgeRepsClaimed ?? [])) // HAUTS FAITS : union globale des trophées déjà payés (jamais re-payés)
    // L'ARCHIVISTE — MIGRATION (one-time) : si le monde ACTIF n'a pas encore de LIGNES (ancien save), on les SEED
    //   depuis tout ce qui a déjà été vu/possédé (seen∪owned) → aucune perte de progression. Idempotent (no-op si déjà rempli).
    seedSeenThisRun([...new Set([...getPokedex().seen, ...ownedCanon])])
    setActiveWorld(aw)
    ...
    try {
        ...
        if (r.ok) {
            ...
            if (shouldCreditEnergyGrant(grantAmt, getActiveWorld())) {
                ...
                if (credited > 0) {
                    logEnergyIncome("✨ Cadeau d'énergie", credited) // JOURNAL : vœu du génie / synergie / admin (persisté avec le crédit)
...
function activeWorldSave(): YellowSave {
    const p = getPlayer()
    const d = getPokedex()
    return { version: SAVE_VERSION, team: p.team, pc: p.pc, items: p.items, reps: p.reps, repsCap: p.repsCap, creditedThrough: p.creditedThrough, pastaBoughtToday: p.pastaBoughtToday, casinoSpentToday: p.casinoSpentToday, pastaDayBonus: p.pastaDayBonus, domeChampionships: p.domeChampionships, pokedex: { seen: d.seen, caught: d.caught, seenAt: d.seenAt, firstCatch: d.firstCatch }, defeatedTrainers: p.defeatedTrainers, rematchedTrainers: p.rematchedTrainers, badges: p.badges, introSeen: p.introSeen, sbireDefeatsToday: p.sbireDefeatsToday, champBattlesToday: p.champBattlesToday ?? 0, consultsToday: p.consultsToday ?? 0, archivisteMatchesToday: p.archivisteMatchesToday ?? 0, archivisteWinsToday: p.archivisteWinsToday ?? 0, badgeRepsClaimed: p.badgeRepsClaimed ?? [], badgeRepsToday: p.badgeRepsToday ?? 0, comparisonConsultsToday: p.comparisonConsultsToday ?? 0, sageSaiyanPointsToday: p.sageSaiyanPointsToday ?? 0, ananasLastBadgeCount: p.ananasLastBadgeCount ?? 0, ananasDate: p.ananasDate ?? "", ananasPeakLevel: p.ananasPeakLevel ?? 0, capturesToday: p.capturesToday ?? 0, sbireWinsTotal: p.sbireWinsTotal, pvpStats: p.pvpStats, domeStats: p.domeStats, domeTierRecord: p.domeTierRecord ?? {}, stats: p.stats, acePeakLevel: p.acePeakLevel, aceBox: p.aceBox, aceTeamSizePeak: p.aceTeamSizePeak, aceWins: p.aceWins, aceDefeatedDate: p.aceDefeatedDate, duelWins: p.duelWins, ownedCts: p.ownedCts, boughtCts: p.boughtCts, gekrocResolved: p.gekrocResolved, hhSpectresShown: p.hhSpectresShown, hhCollectorWins: p.hhCollectorWins, isChampion: p.isChampion, leagueSixShiny: p.leagueSixShiny, mirrorWinHigherLevel: p.mirrorWinHigherLevel, berrySecretKnown: p.berrySecretKnown, berryHarvestDay: p.berryHarvestDay, berryHarvestPicked: p.berryHarvestPicked, sylvebarbeAwake: p.sylvebarbeAwake, caveTradeDone: p.caveTradeDone, goshHintHeard: p.goshHintHeard, orcalineWins: p.orcalineWins, orcalineDate: p.orcalineDate, chenGiftClaims: p.chenGiftClaims, funDefis: p.funDefis, achTrack: p.achTrack, pnj5Wins: p.pnj5Wins, ngplusBattles: p.ngplusBattles, moveReminderUses: p.moveReminderUses, repsBankedTotal: p.repsBankedTotal, welcomeGift: p.welcomeGift, pokerFirstGameDone: p.pokerFirstGameDone, pokerBossStacks: p.pokerBossStacks, pokerCashCap: p.pokerCashCap, pokerCashDate: p.pokerCashDate, spagGift: p.spagGift, pastaGodGift: p.pastaGodGift, labDefi: p.labDefi, customDaemons: p.customDaemons ?? [], ngplusStartedAt: p.ngplusStartedAt, playtimeMs: p.playtimeMs, leaguePotions: p.leaguePotions, ngplusUsed: p.ngplusUsed, run3Used: p.run3Used, ngplusMaitreBeaten: p.ngplusMaitreBeaten, run3StarterBase: p.run3StarterBase, ngplusStarterBase: p.ngplusStarterBase, genieArcSeen: p.genieArcSeen, genesisMode: p.genesisMode, run3Defeated: p.run3Defeated, run3EnergyByArena: p.run3EnergyByArena, caughtThisRun: p.caughtThisRun, seenThisRun: p.seenThisRun, fichesUnlockedThisRun: p.fichesUnlockedThisRun, collectionneurDexGiven: p.collectionneurDexGiven, fusionRoster: p.fusionRoster, fusionNames: p.fusionNames, fusionMoveOrders: p.fusionMoveOrders, fusionTypeChoices: p.fusionTypeChoices, fusionHistory: p.fusionHistory, energyLog: p.energyLog, messageLog: p.messageLog, run3LavapetitSeen: p.run3LavapetitSeen, run3LavapetitCaught: p.run3LavapetitCaught, mimimoyReturned: p.mimimoyReturned, mimimoyAppearances: p.mimimoyAppearances, ballLockRemaining: p.ballLockRemaining, evResetCharges: p.evResetCharges, shinyWish: p.shinyWish, luxeOutcomeQueue: p.luxeOutcomeQueue, bertieOutcomeQueue: p.bertieOutcomeQueue, forcedEncounter: p.forcedEncounter, fusionLeagueCarry: p.fusionLeagueCarry, curseAbundanceStart: p.curseAbundanceStart, curseFreeItemsTaken: p.curseFreeItemsTaken, curseFreeItemDate: p.curseFreeItemDate, potionBuysToday: p.potionBuysToday, jcEnergyBuysToday: p.jcEnergyBuysToday, casinoCapToday: p.casinoCapToday, fusionLeagueDefeats: p.fusionLeagueDefeats, grotteShopBuys: p.grotteShopBuys, chosenAvatar: p.chosenAvatar, clan: p.clan, clanDaemonUid: p.clanDaemonUid, clanCtGiven: p.clanCtGiven, clanTransGiven: p.clanTransGiven, clanVisitDate: p.clanVisitDate, clanTrainByClan: p.clanTrainByClan, craftedItems: p.craftedItems, craftsUsed: p.craftsUsed, craftReady: p.craftReady, megaInLigue: p.megaInLigue, fusionLeagueTryDate: p.fusionLeagueTryDate, fusionChampionRoster: p.fusionChampionRoster, fusionChampionAvatar: p.fusionChampionAvatar, activeWorld: "live", ngplusWorld: null, ngplusOldTeam: null, run3World: null, replayWorld: null, replayRun: null, replayReturn: null }
}
...
export function snapshot(): YellowSave {
    ...
    const givenGlobal = getPlayer().collectionneurDexGiven // L'ARCHIVISTE : dex offert = GLOBAL → répliqué dans tous les mondes
    const claimedGlobal = getPlayer().badgeRepsClaimed ?? [] // HAUTS FAITS : trophées payés = GLOBAL → répliqués partout (jamais re-payés)
    // ARTISANE : compteur À VIE + forge prête = GLOBAUX → même valeur écrite dans TOUS les mondes (comme customDaemons),
    // sinon le plafond se réinitialiserait à chaque bascule de run. (craftedItems reste PER-MONDE : lié aux uid du run.)
    const craftsUsed = getPlayer().craftsUsed
    const craftReady = getPlayer().craftReady
    ...
    const norm = (w: YellowSave | null): YellowSave | null =>
        w ? { ...w, pokedex: { seen: [...pdx.seen], caught: [...pdx.caught], seenAt: pdx.seenAt, firstCatch: pdx.firstCatch }, berrySecretKnown: w.berrySecretKnown || berryKnown, collectionneurDexGiven: w.collectionneurDexGiven || givenGlobal, badgeRepsClaimed: [...new Set([...(w.badgeRepsClaimed ?? []), ...claimedGlobal])], customDaemons: cds, craftsUsed, craftReady, activeWorld: "live" as const, ngplusWorld: null, ngplusOldTeam: null, run3World: null, replayWorld: null, replayRun: null, replayReturn: null, altProfiles: undefined } : null
...
export async function startNewGamePlus(starter: MonInstance, oldTeamFrozen: ChampionMon[]): Promise<boolean> {
    ...
    // 4) DON de départ du run 2. MODE FUN (choix Sartay) : 1000⚡ (plafond de base 1000) + 10 NEXUS-BALLS pour capturer
    //    (les Fusio-Balls ne prennent QUE les fusions → inutiles pour attraper les Daemons sauvages du run 2). NON-FUN :
    //    NGPLUS_START_ENERGY (10000⚡) + plafond aligné (raiseRepsCap AVANT grantReps → pas de rabotage).
    if (getGameMode() === "fun") {
        grantReps(1000)          // reps → 1000 (repsCap de base = 1000, pas de raise)
        addItem("poke_ball", 10) // 10 Nexus-Ball de départ (capture normale)
    } else {
...
export function mergeWorlds(primary: YellowSave, secondary: YellowSave, tag = "fus"): YellowSave {
    ...
    return {
        ...
        domeTierRecord: (() => { // bilan par tier = MAX par tier (les malus anti-farm ne régressent pas au changement d'appareil)
            const out: Record<string, { wins: number; losses: number }> = { ...(primary.domeTierRecord ?? {}) }
            for (const [k, v] of Object.entries(secondary.domeTierRecord ?? {})) {
                const cur = out[k] ?? { wins: 0, losses: 0 }
                out[k] = { wins: Math.max(cur.wins, v.wins), losses: Math.max(cur.losses, v.losses) }
            }
            return out
        })(),
        ...
        // L'ARCHIVISTE : dex offert = GLOBAL monotone → OR des 2 timelines. Les LIGNES vues (per-monde) sont UNIES
        //   pour ne RIEN perdre à la fusion ; les fiches débloquées idem (elles restent consultables).
        collectionneurDexGiven: primary.collectionneurDexGiven || secondary.collectionneurDexGiven,
        seenThisRun: uniq([...(primary.seenThisRun ?? []), ...(secondary.seenThisRun ?? [])]),
        fichesUnlockedThisRun: uniq([...(primary.fichesUnlockedThisRun ?? []), ...(secondary.fichesUnlockedThisRun ?? [])]),
        badgeRepsClaimed: uniq([...(primary.badgeRepsClaimed ?? []), ...(secondary.badgeRepsClaimed ?? [])]), // HAUTS FAITS : payés = union (jamais re-payés)
        badgeRepsToday: 0, // compteur quotidien → repart à zéro dans le monde fusionné
        berryHarvestDay: "",
        berryHarvestPicked: [],
        ...
        // JOURNAL D'ÉNERGIE — simple concat chronologique (secondary puis primary), borné aux plus récentes (pas de dédup : c'est un flux).
        energyLog: [...(secondary.energyLog ?? []), ...(primary.energyLog ?? [])].slice(-ENERGY_LOG_MAX),
        // JOURNAL DES MESSAGES — même politique (concat chronologique borné).
        messageLog: [...(secondary.messageLog ?? []), ...(primary.messageLog ?? [])].slice(-MSG_LOG_MAX),
        ...
        chenGiftClaims: Math.max(primary.chenGiftClaims, secondary.chenGiftClaims),
...
export async function startReplay(run: "run1" | "run2" | "run3", starter: MonInstance | null = null): Promise<boolean> {
    ...
    if (run === "run2") { if (getGameMode() === "fun") { grantReps(1000); addItem("poke_ball", 10) } else { raiseRepsCap(NGPLUS_START_ENERGY - 1000); grantReps(NGPLUS_START_ENERGY) } } // REJEU run 2 = même don que le NG+ initial : fun 1000⚡+10 Nexus-Balls, non-fun 10000⚡
...
/** MODE GENÈSE — démarre un PROFIL neuf « 6 Daemons craftés / zéro capture » : équipe = les créations (niv 5),
 *  customDaemons = leurs specs, genesisMode=true, intro SAUTÉE. Stashe le profil actif (comme startNewProfileFromRun1,
 *  NON-destructif). La Ball est verrouillée jusqu'à la Ligue de Fusion (genesisCaptureLocked) ; la FUSION reste OK. */
export async function startGenesisProfile(team: MonInstance[], customDaemons: StoredCustomDaemon[]): Promise<boolean> {
    if (getActiveWorld() === "replay" || team.length === 0) return false
    if (altProfilesStash.length >= MAX_ALT_PROFILES) return false
    const cur = snapshot()
    const stashedCurrent: YellowSave = { ...cur, altProfiles: undefined }
    const fresh: YellowSave = { ...emptySave(), team, customDaemons, introSeen: true, genesisMode: true, altProfiles: [...(cur.altProfiles ?? []), stashedCurrent] }
    applyServerSave(fresh)
    reregisterCustomDaemons() // les 6 créations doivent être résolvables en combat (getSpecies) dès le départ
    await persistNow()
    return true
}
